	...
	private:
		...
		void DrawComponent(const std::string& name, ImguiBlock uiblock)
		{
			...
			if (m_SelectedEntity.HasComponent<T>())
			{
				...
				// Comenzamos un grupo para poder posicionar el bot�n absolutamente
				ImGui::BeginGroup();

				// Calculamos el ancho del header menos el espacio para el bot�n
				float headerWidth = ImGui::GetContentRegionAvail().x - 30;

				// Dibujamos el header con un ancho espec�fico
				bool open = ImGui::CollapsingHeader(name.c_str(), ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_AllowItemOverlap);

				// Posicionamos el bot�n a la derecha del header
